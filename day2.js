...
function day2b() {
    ...
    //gets the primiter and volume
    for (var i = 0; i < myValues.inputArray.length; i++) {//sets an revolution counter, ensures that that the revolution nubmer is less than the index number for the inputArray, and then increases the revolution counter
        var equationArray = [];// creates an empty array which is reset each time 
        equationArray = myValues.inputArray[i].split("x");//puts the current element in the equationArray as 3 seperate elements without the "x"'s

        var side1 = (2 * equationArray[0] + 2 * equationArray[1]);
        var side2 = (2 * equationArray[1] + 2 * equationArray[2]);
        var side3 = (2 * equationArray[2] + 2 * equationArray[0]);
        // Since the smallest side must be added to the final equation this variable is used to find the smallest one (by area)
        var smallestSide = Math.min(side1, side2, side3);
        var vol = equationArray[0] * equationArray[1] * equationArray[2];
        var primVol = smallestSide + vol;

        additionArray.push(primVol);
